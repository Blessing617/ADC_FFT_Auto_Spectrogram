...
void Get_Show_THD(void)
{
	...
	printf("t14.txt=\"%.2f\"\xff\xff\xff",THD);
	printf("t15.txt=\"%.2f\"\xff\xff\xff",max[0]/max[0]);
	printf("t16.txt=\"%.2f\"\xff\xff\xff",max[1]/max[0]);
	printf("t17.txt=\"%.2f\"\xff\xff\xff",max[2]/max[0]);
	printf("t18.txt=\"%.2f\"\xff\xff\xff",max[3]/max[0]);
	printf("t19.txt=\"%.2f\"\xff\xff\xff",max[4]/max[0]);
...
{  
	...
	printf("t10.txt=\"%dHz\"\xff\xff\xff",FRE);
	printf("page0.pic=\"0\"\xff\xff\xff");
	
	for(i=1;i<340;i++)
	{
		posX1 = i;
		posX2 = posX1 + 1;
		posY1 = 135 - (ADC_GetValue[i] 	 * (135.0f / 4096.0f));
		posY2 = 135 - (ADC_GetValue[i+1] * (135.0f / 4096.0f));
		printf("line %d,%d,%d,%d,%d\xff\xff\xff", posX1, posY1, posX2, posY2, 63488);
		
		posY1 = 270;
		posY2 = 270 - ((fft_outputbuf[3*i]+fft_outputbuf[3*i+1]+fft_outputbuf[3*i+2])/3 * (2048.0f / 4096.0f));
	...
	printf("t12.txt=\"%.0fHz\"\xff\xff\xff",HZ);
	printf("t13.txt=\"%.2fV\"\xff\xff\xff",amp);
